		    						...
		    						<tr>
		    							<td>
											...
											<% } %>
		    									...
		    									<!--  <img class="product-image" src="<%=kkEng.getProdImageBase(item.getProduct())+"_1_tiny"+kkEng.getProdImageExtension(item.getProduct())%>" border="0" alt="<%=item.getProduct().getName()%>" title="<%=item.getProduct().getName()%>"/>  -->
		    								  
		    								  	<!-- jk  add link to Etilize images - product ID stored in  custom1Int -->
		    								
		    								<% com.konakart.appif.ProductIf prod =  item.getProduct();%>
		    								
		    								<img class="product-image" src="<%=kkEng.getProdImageBase(item.getProduct()).replaceAll("null", "") + "100/100/" + prod.getCustom1Int() + ".jpg" %>" border="0" alt="<%=item.getProduct().getName()  %>" title="<%=item.getProduct().getName()%>"/>  
										...
										<% } %>		    							
										<!-- jk  add local image http://localhost:8780/konakart/images/ -->
<%-- 		    							<td class="center"><a href='<%="EditCartSubmit.action?action=r&id="+item.getId()%>' class="has-tooltip" 
												title='<%=kkEng.getMsg("common.remove.item")%>'><img src="<%=kkEng.getImageBase()%>/x-button.png" border="0"></a></td>  --%>

 			    							<td class="center"><a href='<%="EditCartSubmit.action?action=r&id="+item.getId()%>' class="has-tooltip" 
 			    								title='<%=kkEng.getMsg("common.remove.item")%>'><img src="<%=kkEng.getLocalImageBase()%>/x-button.png" border="0"></a></td>
