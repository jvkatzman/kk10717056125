	...
	<div id="side-menu">	
  			<% if (prodMgr.getCurrentCategoriesLength() > 0){%>				
				...
				<!-- //jk changed 2/18/2014 -->					
					...
					<div id="remove-all"><img  src="<%=kkEng.getLocalImageBase()%>/x-button.png"><a href='<%="RemoveTags.action?t="+prodMgr.getProdTimestamp()%>'><kk:msg  key="products.body.clear.filters"/></a></div>				
				<%}%>
				...
				<%if (haveFacets){ %>
					...
					<% for (int i = 0; i < prodMgr.getCurrentTagGroups().length; i++){ %>
						...
						<!-- //jk changed 3/11/2014 -Eliminated category name on facet search -->
						<%-- <h2><%=tagGroup.getName() %> </h2> --%>
						
						<%Integer intMax =  tagGroup.getName().indexOf('!') ; %>
						<%if (intMax < 0) { intMax=999; }  %>
						<h2><%=tagGroup.getName().substring(0,   Math.min(intMax, tagGroup.getName().length())) %> </h2>
