...
function kkSearch() {
	...
	if (key != null && key.length > 0) {
		...
		// Make konakart search use the entered value instead of suggestions
                        document.getElementById('searchText').value = text;
                        document.getElementById('ssForm').action = getURL("QuickSearch.action");
                        document.getElementById('ssForm').submit();
	}
...
var addToCartCallback = function(result, textStatus, jqXHR) {
	...
	if (result.items != null && result.items.length > 0) {
		...
		for ( var i = 0; i < result.items.length; i++) {
			...
			txt += '<a href="'+ getURL("SelectProd.action", new Array("prodId",item.product.id)) +'"><img src="'+getProdImageBase(item.product,result.imgBase)+'_1_tiny'+getProdImageExtension(item.product)+'" border="0" alt="'+item.product.name+'" title="'+item.product.name+'"></a>';
			txt += '<a href="'+ getURL("SelectProd.action", new Array("prodId",item.product.id)) +'" class="shopping-cart-item-title">'+item.product.name+'</a>';
