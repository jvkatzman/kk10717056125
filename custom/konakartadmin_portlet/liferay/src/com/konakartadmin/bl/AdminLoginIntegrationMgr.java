        ...
        AdminLoginIntegrationMgrInterface
{
    ...
    {
        /**
         * Note that this customization overrides credential checking and so if the Admin App is
         * available directly from a URL rather than through Liferay then all security will be
         * disabled. In order to implement tighter security you should use an SSO system and pass a
         * token through to this method so that it can check with the SSO service whether the token
         * is valid.
         */
        if (log.isDebugEnabled())
        {
            log.debug("returning 1 => KonaKart will not check login credentials");
        }
        return 1;
    }
}
